...
import {submitForm} from "../../pages/auth/helper/SubmitForm";
...
const CampusesAdd = () => {
  ...
  const onSubmit = (event) => {
    debugger
    let url = window.abeApp.urls.api.CAMPUSES_ADD
    if (isUpdate) {
      url = window.abeApp.urls.api.CAMPUSES_EDIT.replace(':id', params.id)
    }
    submitForm({
      event,
      state,
